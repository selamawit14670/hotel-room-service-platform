...
export default function ProblemSection() {
  ...
  return (
    <section className="relative py-12 md:py-24 bg-slate-50 dark:bg-[#030408] transition-colors duration-300" id="problem-solutions">
      {/* Absolute Gradient Accents */}
      <div className="absolute top-0 right-1/4 w-80 h-80 bg-red-950/5 dark:bg-red-950/10 rounded-full blur-3xl pointer-events-none"></div>
      <div className="absolute bottom-0 left-10 w-80 h-80 bg-cyan-950/5 dark:bg-cyan-950/10 rounded-full blur-3xl pointer-events-none"></div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center mb-16">
          <div className="lg:col-span-8">
            <span className="inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs font-mono uppercase text-red-500 dark:text-red-400 bg-red-550/10 border border-red-500/20 mb-3">
              <AlertCircle className="w-3.5 h-3.5" /> Operations Reality Check
            </span>
            <h2 className="font-display text-3xl md:text-5xl font-extrabold text-slate-800 dark:text-slate-100 tracking-tight leading-tight">
              Hotels Lose Substantial Revenue <br />
              Through Outdated Room Service
            </h2>
            <p className="mt-4 text-slate-600 dark:text-slate-400 text-sm md:text-base max-w-2xl font-normal leading-relaxed">
          ...
          <div className="lg:col-span-4 grid grid-cols-2 gap-4">
            <div className="p-4 rounded-xl border border-slate-200 dark:border-slate-900 bg-white dark:bg-neutral-950 text-center transition-colors duration-300">
              <span className="text-3xl font-bold font-display text-red-500">72%</span>
            ...
            <div className="p-4 rounded-xl border border-slate-200 dark:border-slate-900 bg-white dark:bg-neutral-950 text-center transition-colors duration-300">
              <span className="text-3xl font-bold font-display text-luxury-gold">+37%</span>
        ...
        <div className="flex justify-center mb-12">
          <div className="inline-flex p-1.5 rounded-xl bg-slate-200/50 dark:bg-neutral-950 border border-slate-300/30 dark:border-slate-900 transition-colors duration-300">
            <button
              onClick={() => setActiveTab('traditional')}
              className={`px-6 py-2 rounded-lg text-xs font-semibold tracking-wide transition-all cursor-pointer ${
                activeTab === 'traditional'
                  ? 'bg-red-500/15 border border-red-500/30 text-red-650 dark:text-red-400'
          ...
          {activePoints.map((point, i) => (
            <div 
              ...
              className={`p-5 rounded-2xl border ${
                activeTab === 'traditional' 
                  ? 'border-slate-200 dark:border-slate-900/40 hover:border-red-500/25 bg-white dark:bg-black' 
                  : 'border-slate-200 dark:border-slate-900/40 hover:border-luxury-gold/25 bg-white dark:bg-[#03060d]'
              } transition-luxury relative group overflow-hidden flex flex-col justify-between transition-colors duration-300`}
            >
              ...
              <div>
                ...
                <h3 className="font-display text-sm font-bold text-slate-800 dark:text-slate-200 tracking-tight leading-snug">
                  {point.title}
                </h3>
                <p className="mt-2 text-slate-600 dark:text-slate-400 text-xs leading-relaxed font-sans">
                  {point.desc}
                </p>
              </div>

              <div className="mt-6 pt-3 border-t border-slate-100 dark:border-slate-900/60 flex items-center justify-between">
